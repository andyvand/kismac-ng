...
bool RTL8187Jack::_massagePacket(UInt16 len) {
    ...
    frame.ctrl.len = (flags & 0xFFF) - 4;
    frame.ctrl.rate = (flags >> 20) & 0xF;
	signal = hdr->agc >> 1;
	if (frame.ctrl.rate > 3) {	/* OFDM rate */
		if (signal > 90)
			signal = 90;
		else if (signal < 25)
			signal = 25;
    ...
    frame.ctrl.signal = signal;
    frame.ctrl.silence = 0;
